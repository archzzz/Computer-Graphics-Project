...
void IntersectionUI::changeShape( ShapesUI::ShapeType type )
{
	...
	case ShapesUI::SHAPE_SPHERE : {
		shape = &sphere; 
		shape->NewTesselation(5,5); } break;
	case ShapesUI::SHAPE_CYLINDER : {
		shape = & cylinder; 
		shape->NewTesselation(50,2); }break;
	case ShapesUI::SHAPE_CONE : {
		shape = &cone; 
		shape->NewTesselation(50,2); }break;
	case ShapesUI::SHAPE_CUBE : {
		shape = &cube; 
		shape->NewTesselation(2,2); }break;
	case ShapesUI::SHAPE_BUNNY	 : {
		shape = &bunny; 
		shape->NewTesselation(2,2); }break;
	}
}
...
void IntersectionUI::writeTest() const {
	...
	for (int i = seed; i < (seed + 1638); i++) {
		...
		Cube newCube;
		newCube.intersect(p,dir);
		cubeHr = newCube.hr; 



		Cylinder newCylinder;
		newCylinder.intersect(p, dir);
		cylinderHr = newCylinder.hr;

		Cone newCone;
		newCone.intersect(p, dir);
		coneHr = newCone.hr;

		Sphere newSphere;
		newSphere.intersect(p, dir);
		sphereHr = newSphere.hr;
